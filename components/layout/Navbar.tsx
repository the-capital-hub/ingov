...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled && !mobileMenuOpen ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        <div className="px-10 flex items-center justify-between">
          <Link href="/" className="group flex flex-col items-start shrink-0" onClick={e => handleNavClick(e, '/')}>
            <span className={`font-heading font-black text-3xl tracking-tight transition-colors ${(scrolled && !mobileMenuOpen) ? 'text-highlight' : 'text-white'}`}>
              InGov
            </span>
            <span className={`text-[10px] uppercase font-bold tracking-[0.1em] mt-[-4px] transition-colors ${(scrolled && !mobileMenuOpen) ? 'text-highlight' : 'text-white'}`}>
              by iProfit
            </span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-8">
            <div className={`flex gap-8 items-center rounded-full px-8 py-3 transition-colors ${scrolled ? 'bg-bgbase border border-border' : 'bg-white/10 backdrop-blur-sm'}`}>
              {links.map(link => (
                <Link
                  key={link.label}
                  href={link.href}
          ...
          <div className="hidden lg:flex shrink-0">
            <Link href="/book-demo" className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-white hover:bg-white/80 text-black font-bold px-6 py-2.5 rounded-full shadow-lg transition-all hover:-translate-y-0.5 active:translate-y-0 text-sm">
              Book a Free Demo
            </Link>
          </div>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col gap-8 text-left">
              ...
              <div className="mt-auto pb-10">
                <p className="text-white/80 text-xs font-bold uppercase tracking-[0.2em] mb-4">InGov by iProfit</p>
